...
.admin-banner-preview { display: grid; width: 180px; aspect-ratio: var(--banner-aspect-ratio, 16 / 9); place-items: center; overflow: hidden; border: 1px solid #8292a8; background: #071a32; }
.admin-banner-preview img, .admin-banner-preview video { width: 100%; height: 100%; object-fit: contain; }
.admin-media-error { padding: 14px; color: #ffcfcb; font: 11px var(--mono); text-align: center; }
...
<section class="admin-workspace">
    ...
    <div class="admin-banner-list">
        @forelse ($banners as $banner)
            @php($isVideo = $banner->media_type === 'video' || in_array(strtolower(pathinfo($banner->media_path, PATHINFO_EXTENSION)), ['mp4', 'mov', 'webm'], true))
            <div class="admin-banner-item"><div class="admin-banner-preview" data-banner-preview>@if ($isVideo)<video src="{{ asset('storage/'.$banner->media_path) }}" muted controls preload="auto" playsinline></video>@else<img src="{{ asset('storage/'.$banner->media_path) }}" alt="{{ $banner->title }}">@endif</div><div><h2>{{ $banner->title ?: 'Banner tanpa judul' }}</h2><small>{{ strtoupper($isVideo ? 'video' : 'image') }} · {{ $banner->is_active ? 'AKTIF DI BERANDA' : 'TIDAK AKTIF' }}</small><form class="admin-banner-form" method="POST" action="{{ route('admin.banners.update', $banner) }}" enctype="multipart/form-data">@csrf @method('PATCH')<label class="admin-title-field">Judul banner (opsional)<input type="text" name="title" value="{{ $banner->title }}"></label><label>Ganti media<input type="file" name="media" accept="image/jpeg,image/png,image/webm,video/mp4,video/quicktime,video/webm" data-banner-media></label><label class="admin-banner-check"><input type="checkbox" name="is_active" value="1" @checked($banner->is_active)> Tampilkan</label><button type="submit">Simpan</button></form><form class="admin-banner-form" method="POST" action="{{ route('admin.banners.destroy', $banner) }}" onsubmit="return confirm('Hapus banner ini?')">@csrf @method('DELETE')<button class="admin-delete" type="submit">Hapus banner</button></form></div></div>
        @empty
