...
if (requestPackageButtons.length) {
    ...
    drawerCustomer.placeholder = "Nama:";
    ...
    const requestRenderCart = () => {
        ...
        requestDrawer.querySelectorAll('[data-item-field="primary"]').forEach((field) => { field.closest("label").firstChild.textContent = "Tema"; field.removeAttribute("required"); field.placeholder = "Tema:"; });
        drawerCart.querySelectorAll('[data-item-field="notes"]').forEach((field) => field.closest("label").remove());
        drawerCart.querySelectorAll('[data-item-field="secondary"]').forEach((field) => { const packageName = field.closest(".request-cart-item")?.querySelector("strong")?.textContent || ""; if (!packageName.startsWith("AI Video")) field.closest("label").remove(); });
    ...
    requestDrawer.querySelector("[data-drawer-continue]").addEventListener("click", () => { if (!requestCart.length || !drawerCustomer.checkValidity() || [...requestDrawer.querySelectorAll("[data-item-field][required]")].some((field) => !field.checkValidity())) { drawerCustomer.reportValidity(); return; } requestDetails.elements.customer_name.value = drawerCustomer.value; requestBuildInvoice(); closeRequestDrawer(); requestCheckout.hidden = false; document.body.style.overflow = "hidden"; requestShowStep(4); });
